...
/*
Npc.cs
- Wandelt over een Path3D in Neutral state
- Detecteert speler binnen 50.0f → Alert state (uitroepteken + rennen)
- Achtervolgt speler in Alert state
- Gaat terug naar wandelen als speler uit zicht is
*/
...
public partial class Npc : CharacterBody3D
{
    ...
    [Export] public float DetectionRadius = 50.0f; // Afstand waarop NPC speler ziet
    [Export] public float CatchDistance = 1.5f;
    [Export] public bool DebugLos = true;
    ...
    [Export] public float AlertDuration = 5.0f; // Hoe lang rennen voor terugkeer naar wandelen
    ...
    private Path3D _npcPath;
    private int _pathDirection = 1; // 1 = vooruit, -1 = achteruit
    private float _alertTimeLeft = 0f;
    private bool _wasChasing = false;

    // ── Externe control (van teamgenoot) ─────────────────────────────────────
    private bool _externallyControlled = false;
    ...
    public override void _Ready()
    {
        ...
        // Zoek het pad in de scene
        _npcPath = GetTree().CurrentScene.GetNodeOrNull<Path3D>("NPCPath");
        if (_npcPath != null)
        {
            _pathFollow = _npcPath.GetNodeOrNull<PathFollow3D>("PathFollow");
            if (_pathFollow == null)
            {
                _pathFollow = new PathFollow3D();
                _pathFollow.Name = "PathFollow";
                _npcPath.AddChild(_pathFollow);
            }
            _pathFollow.Loop = true;
            GD.Print($"{Name}: Pad gevonden, start wandelen.");
        }
        else
        {
            GD.Print($"{Name}: Geen NPCPath gevonden, NPC staat stil.");
        }
    }
    ...
    private void NeutralTick(float dt)
    {
        // Check of speler binnen DetectionRadius is
        _target = FindVisiblePlayer();
        if (_target != null)
        {
    ...
    private void AlertTick(float dt)
    {
        ...
        // Terug naar Neutral als timer afloopt en speler uit zicht
        if (_alertTimeLeft <= 0f && _target == null)
        {
            ChangeState(NpcState.Neutral);
            return;
        }
        ...
        if (_target == null)
        {
            ...
            if (_target == null)
            {
                // Blijf rennen in dezelfde richting
                Vector3 dir = -Transform.Basis.Z;
                ApplyMove(GlobalPosition + dir * 10f, RunSpeed, dt);
                UpdateHeadPitch(0f, dt);
                return;
            }
        }
        ...
        {
            _alertTimer?.Stop();
            _alertTimeLeft = AlertDuration; // Reset timer
            ChaseTarget(dt);
        }
        else
        {
    ...
    // ── Wandelen over pad ────────────────────────────────────────────────────
    private void WalkAlongPath(float dt)
    {
        if (_npcPath == null || _pathFollow == null) return;

        // Beweeg de PathFollow over het pad
        _pathFollow.Progress += WalkSpeed * _pathDirection * dt;

        // Check of we aan het einde van het pad zijn (als loop uit staat)
        if (!_npcPath.Curve.GetPointCount().Equals(null))
        {
            float maxProgress = _npcPath.Curve.GetBakedLength();
            if (_pathFollow.Progress >= maxProgress || _pathFollow.Progress <= 0f)
            {
                _pathDirection *= -1; // Keer om
                _pathFollow.Progress = Mathf.Clamp(_pathFollow.Progress, 0f, maxProgress);
            }
        }

        // NPC volgt de PathFollow
        Vector3 targetPos = _pathFollow.GlobalPosition;
        ApplyMove(targetPos, WalkSpeed, dt);
    }
    ...
    {
        _hasCaught = false;
        _alertTimeLeft = AlertDuration;
        _alertTimer?.Stop();
        ChangeState(NpcState.Alert);
        GD.Print($"{Name} → RENNEN! Speler gezien binnen {DetectionRadius}m");
    }
    ...
    private void ChaseTarget(float dt)
    {
        ...
        if (dist2d <= CatchDistance)
        {
            ...
            ApplyMove(_target.GlobalPosition, RunSpeed, dt);
        }
        else
        {
            ApplyMove(_target.GlobalPosition, RunSpeed, dt);
